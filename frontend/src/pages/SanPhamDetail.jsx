...
export default function SanPhamDetail() {
  ...
  const renderDetail = (html) => {
    if (!html) return null
    return html
      .replace(/<h3>/g, '<h3 style="color:#4A2C17;font-weight:700;font-size:1rem;margin-top:1.25rem;margin-bottom:0.5rem;border-left:3px solid #E07820;padding-left:8px;">')
      .replace(/<ul>/g, '<ul style="list-style-type:disc;padding-left:1.5rem;margin-bottom:1rem;">')
      .replace(/<li>/g, '<li style="margin-bottom:0.35rem;font-size:0.8rem;">')
  }
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2">
            <div className="grid md:grid-cols-2 gap-8 mb-10">
              <div>
                <img src={product.image} alt={displayName} className="w-full rounded-lg shadow-md object-cover" style={{ height: '280px' }} />
              </div>
              <div>
                ...
                <h2 className="text-2xl font-black text-[#4A2C17] mt-3 mb-3">{displayName}</h2>
                <p className="text-gray-600 leading-relaxed mb-5 text-sm">{displayDesc}</p>

                <div className="space-y-2.5 mb-6">
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-semibold text-gray-700 w-24 shrink-0">{t.origin_lbl}:</span>
                    <span className="text-gray-600">{product.origin}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-semibold text-gray-700 w-24 shrink-0">{t.unit_lbl}:</span>
                ...
                <div className="flex flex-wrap gap-3">
                  <a href="https://wa.me/+84986768378" target="_blank" rel="noreferrer"
                    className="bg-[#25D366] hover:bg-[#1da857] text-white px-4 py-2.5 font-semibold flex items-center gap-2 text-sm transition-colors">
                    <FaWhatsapp size={15} /> {t.wa_btn}
                  </a>
                  <a href="mailto:Artocavn@gmail.com"
                    className="border-2 border-[#4A2C17] text-[#4A2C17] px-4 py-2.5 font-semibold hover:bg-[#4A2C17] hover:text-white transition-colors flex items-center gap-2 text-sm">
            ...
            {displayDetail && (
              <div className="bg-white border border-gray-200 rounded-lg p-6 mb-8">
                <h3 className="font-black text-[#4A2C17] text-xl mb-5 pb-3 border-b-2 border-[#E07820]">{t.detail_title}</h3>
                <div
            ...
            {related.length > 0 && (
              <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                  <h3 className="font-bold text-gray-800 text-sm">{t.related_title}</h3>
                </div>
                  ...
                  {related.map(p => (
                    <Link key={p.id} to={`/san-pham/${p.slug}`}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-700 group-hover:text-[#4A2C17] transition-colors truncate">
                          {lang === 'en' && p.name_en ? p.name_en : p.name}
                        </p>
                        <p className="text-xs text-gray-400 mt-0.5">{p.origin}</p>
                      </div>
                      <FiArrowRight className="text-gray-400 group-hover:text-[#E07820] shrink-0" size={14} />
                    </Link>
                  ))}
                </div>
              </div>
